...
pub fn forecast_daily_gru<B: Backend>(
    ...
    for _day in 0..forecast_days {
        ...
        // Update the datetime (add one day)
        // This is a simplification - in a real application, we would need to
        // handle weekends, holidays, etc.
        let datetime_series = new_row.column("datetime")?.clone();

        // Add one day in microseconds (86400000000)
        // This requires transforming the datetime to int64, adding, then converting back
        let datetime_int = datetime_series.cast(&DataType::Int64)?;
        let datetime_values: Vec<i64> = datetime_int
            .i64()?
            .into_iter()
