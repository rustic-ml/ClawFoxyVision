...
impl<B: Backend> LSTM<B> {
    ...
    fn process_direction(
        ...
        for t in 0..seq_len {
            ...
            let i_gate = gates.clone().narrow(1, 0, 1).reshape([batch_size, self.hidden_size]);
            let f_gate = gates.clone().narrow(1, 1, 1).reshape([batch_size, self.hidden_size]);
            let g_gate = gates.clone().narrow(1, 2, 1).reshape([batch_size, self.hidden_size]);
            let o_gate = gates.narrow(1, 3, 1).reshape([batch_size, self.hidden_size]);
            ...
            output_sequence = output_sequence.slice_assign(
                ...
                h.clone().unsqueeze::<3>().reshape([batch_size, 1, self.hidden_size])
            );
        }
...
mod tests {
    ...
    fn test_lstm_creation() {
        ...
        let lstm: LSTM<NdArray> = LSTM::new(10, 20, 1, false, &device);
        ...
        // For input weights: [gate_size, input_size] = [4 * hidden_size, input_size]
        assert_eq!(lstm.input_weights.weight.dims(), [10, 80]); 
        // For hidden weights: [gate_size, hidden_size] = [4 * hidden_size, hidden_size]
        assert_eq!(lstm.hidden_weights.weight.dims(), [20, 80]);
    }
    
    #[test]
    fn test_bidirectional_lstm_creation() {
        let device = NdArrayDevice::default();
        let lstm: LSTM<NdArray> = LSTM::new(10, 20, 1, true, &device);
        
        // Check basic parameters
        assert_eq!(lstm.bidirectional, true);
        
        // Check reverse weight dimensions
        assert!(lstm.reverse_input_weights.is_some());
        assert!(lstm.reverse_hidden_weights.is_some());
        
        // Check dimensions of reverse weights
        // Should be same as forward weights: [gate_size, input_size]
        assert_eq!(
            lstm.reverse_input_weights.as_ref().unwrap().weight.dims(),
            [10, 80]
        );
        assert_eq!(
            lstm.reverse_hidden_weights.as_ref().unwrap().weight.dims(),
            [20, 80]
        );
    }
    ...
    fn test_lstm_forward() {
        ...
        // Create a simple input tensor [batch_size, seq_len, input_size]
        let batch_size = 3;
        let seq_len = 5;
        let input_size = 10;
        
        let input = Tensor::<NdArray, 3>::zeros([batch_size, seq_len, input_size], &device);
        
        // Test forward pass
        let output = lstm.forward(input);
        
        // Check output shape [batch_size, seq_len, hidden_size]
        assert_eq!(output.dims(), [batch_size, seq_len, 20]);
    }
    
    #[test]
    fn test_bidirectional_lstm_forward() {
        let device = NdArrayDevice::default();
        let lstm: LSTM<NdArray> = LSTM::new(10, 20, 1, true, &device);
        ...
        // Check output shape [batch_size, seq_len, 2 * hidden_size] for bidirectional
        assert_eq!(output.dims(), [batch_size, seq_len, 40]);
    }
} 