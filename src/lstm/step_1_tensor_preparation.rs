...
pub fn split_data(df: &DataFrame, validation_split: f64, time_based: bool) -> Result<(DataFrame, DataFrame)> {
    // Validate input
    if df.height() == 0 {
...
pub fn impute_missing_values(
    ...
    for &col in columns {
        ...
        let imputed_series = match strategy {
            ...
            "forward_fill" => {
                forward_fill_series(&f_series, col)
            },
            "backward_fill" => {
                backward_fill_series(&f_series, col)
            },
            _ => {
                return Err(PolarsError::ComputeError(
        ...
        match (method, strategy) {
            ("zscore", "clip") => {
                if let (Some(mean), Some(std_dev)) = (f_series.mean(), f_series.std(1)) {
                    ...
                    let clipped: Vec<f64> = f_series.into_iter()
                        .map(|opt_v| {
                            if let Some(v) = opt_v {
            ...
            ("iqr", "clip") => {
                ...
                let mut values: Vec<f64> = f_series.into_iter()
            ...
            _ => {
                return Err(PolarsError::ComputeError(
                    format!("Unsupported method/strategy combination: {}/{}", method, strategy).into()
                ));
            }
        }
    }
...
pub fn dataframe_to_tensors<B: Backend>(
    ...
    if nan_count > 0 {
        ...
        if final_nans > 0 {
            ...
            return dataframe_to_tensors::<B>(&df_clean, sequence_length, forecast_horizon, device, use_extended_features, batch_size);
        }
    ...
    let columns: Vec<ChunkedArray<Float64Type>> = df.get_columns()
        .iter()
        .map(|col| col.f64().unwrap().clone())
        .collect();
    
    // Find the index of the close column if it exists
    let close_idx = feature_columns.iter().position(|&s| s == "close").unwrap_or(0);
...
pub fn dataframe_to_diff_tensors<B: Backend>(
    ...
    dataframe_to_tensors::<B>(&diff_df, sequence_length, forecast_horizon, device, use_extended_features, None)
}
...
pub fn build_burn_lstm_model(
    ...
    normalize_features(&mut df_norm, &["close", "open", "high", "low"], false, false)?;
    
    // Create tensors using normalized data
    dataframe_to_tensors::<BurnBackend>(&df_norm, crate::constants::SEQUENCE_LENGTH, forecast_horizon, &device, false, None)
