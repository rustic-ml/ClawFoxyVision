...
use predict_price_lstm::daily::lstm::step_4_train_model::train_daily_lstm;
use predict_price_lstm::daily::lstm::step_5_prediction::{generate_forecast, predict_next_day};
use predict_price_lstm::daily::lstm::step_6_model_serialization::{
    load_daily_lstm_config, load_daily_lstm_model, save_daily_lstm_config, save_daily_lstm_model,
};
use std::path::Path;
...
fn main() -> Result<()> {
    ...
    let config = DailyLSTMModelConfig::new(input_size, hidden_size, output_size, dropout_rate);
    ...
    println!("LSTM Forecast (untrained model):");
    ...
    // Create optimizer manually as train_daily_lstm expects an optimizer
    ...
    // Save model
    save_daily_lstm_model(&trained_model, &model_path, &config)?;
    ...
    let loaded_config = load_daily_lstm_config(&config_path)?;
    let loaded_model = load_daily_lstm_model::<Backend, _>(&loaded_config, &model_path, &device)?;
