...
use predict_price_lstm::minute::gru::{step_4_train_model, step_5_prediction};
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    println!(
        "Close price range in training data - min: ${:.2}, max: ${:.2}",
        min_val, max_val
    );
    ...
    // Train GRU model
    let (trained_gru, _) =
        step_4_train_model::train_gru_model(features, targets, training_config, &device)?;
    ...
    println!(
        "Generating multi-step predictions for {} days...",
    ...
    for step in 0..target_forecast_horizon {
        ...
        for col_name in TECHNICAL_INDICATORS.iter() {
            if current_df.schema().contains(col_name) {
                ...
                return Err(
                    format!("Required column '{}' not found in test data", col_name).into(),
                );
            }
        }
        ...
        let step_predictions =
            step_5_prediction::predict_next_step(&trained_gru, step_input, &device, false)?;
    ...
    println!(
        "Total forecasted change after {} days: {:.2}%",
        target_forecast_horizon, overall_change
    );
